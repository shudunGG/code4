 ...
 */
@Controller
@RequestMapping("ssqd/bmsx")
public class BumenshixiangController {
	@Autowired
	private ShishiqingdanService shishiqingdanService;
	public AccessRecordService acceRecordService = SpringUtil.getBean("AccessRecordService",AccessRecordService.class);
	@Autowired
	 ...
	 * @return
	 */
	@RequestMapping(value = "bmsxlist")
	public ModelAndView xzsplist(String webid,String deptCode) {
		ModelAndView modelAndView = new ModelAndView();
		if(!FilterUtil.isSqlOK(webid) || NumberUtil.getInt(webid) <= 0 || !FilterUtil.isNumeric(webid)||!FilterUtil.isSqlOK(deptCode)){
            RedirectView redirectView = new RedirectView(BaseInfo.getContextPath() +"/error/error.do");
            modelAndView.setView(redirectView);
            return modelAndView;
        }
		 List<Theme> gerenthemelist = (List) CacheUtil.getValue("ziranrentheme",
				  GszwStaticValues.ZIRANRENTHEM); 
		List<Theme> farenthemelist = (List)
			CacheUtil.getValue("farentheme", GszwStaticValues.FARENTHEM);
		DataSourceSwitch.change("2");
		List<Dept> deptlist = shishiqingdanService.getDeptsNameAndCode(webid);
		
		List<Shishiqingdan> typelist=shishiqingdanService.getTypeIdAanName(webid);
		//获取js
				String top="";
				String banner="";
				String nav="";
		...
		modelAndView = new ModelAndView("gszw/bscx/ssqd/bumenshixiang/bumenshixiang_list");
		modelAndView.addObject("gerenthemelist", gerenthemelist);
		modelAndView.addObject("farenthemelist", farenthemelist);
	 ...
	 * @return
	 */
	@RequestMapping(value = "bmsxdown")
	public ModelAndView bmsxdown(String gerenthemeId,String deptCode,String isonline,String keyword,
		...
		if(!FilterUtil.isSqlOK(gerenthemeId)||!FilterUtil.isSqlOK(deptCode)||!FilterUtil.isSqlOK(isonline)
				||!FilterUtil.isSqlOK(keyword)
				||!FilterUtil.isSqlOK(number)
				||!FilterUtil.isSqlOK(numOfOnePage)
				||!FilterUtil.isSqlOK(webid)||!FilterUtil.isSqlOK(typeId)){
			Date date  =new Date();
			System.out.println("date: "+date.toString()+"gerenthemeId: "+gerenthemeId+"typeId:"+typeId+"deptCode: "+deptCode+
		...
		ModelAndView modelAndView = new ModelAndView();
		DataSourceSwitch.change("2");
		List<Shishiqingdan> shishiqingdanlist = shishiqingdanService.getBumenshixiangShishiqingdanList(gerenthemeId,typeId,deptCode,
		...
		for(Shishiqingdan shishiqingdan:shishiqingdanlist){
			List<Yewubanlixiang> yewubanlixiangList = shishiqingdanService.getBumenshixiangYewubanlixiangList(shishiqingdan.getTaskCode());
			shishiqingdan.setYewubanlixiangList(yewubanlixiangList);
			shishiqingdanHavaYewubanlixianglist.add(shishiqingdan);
		}
		int count = shishiqingdanService.getBumenshixiangShishiqingdanCount(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		if(count>0){
		...
		int count1 = shishiqingdanService.getBumenshixiangShishiqingdanCount1(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		int count2 = shishiqingdanService.getBumenshixiangShishiqingdanCount2(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		int count3 = count1-count2;
		int shishiqindganAndYewubanlixiangcount = shishiqingdanService.getBumenshixiangShishiqingdanAndYewubanlixiangCount(gerenthemeId,deptCode,isonline,keyword,webid);
		modelAndView = new ModelAndView("gszw/bscx/ssqd/bumenshixiang/bumenshixiang_down");
		...
		modelAndView.addObject("webid", webid);
		modelAndView.addObject("shishiqindganAndYewubanlixiangcount", shishiqindganAndYewubanlixiangcount);
		return modelAndView;
	}
	/**
	 * 没有参数的部门事项页面
	 * @return
	 */
	@RequestMapping(value = "meiyoucanshudebmsxdown")
	public ModelAndView meiyoucanshudebmsxdown() {
		int pageNum = 0;
		ModelAndView modelAndView = new ModelAndView();
		DataSourceSwitch.change("2");
		List<Shishiqingdan> shishiqingdanlist = shishiqingdanService.getmeiyoucanshuBumenshixiangShishiqingdanList();
		List<Shishiqingdan> shishiqingdanHavaYewubanlixianglist = new ArrayList<Shishiqingdan>();
		for(Shishiqingdan shishiqingdan:shishiqingdanlist){
			List<Yewubanlixiang> yewubanlixiangList = shishiqingdanService.getBumenshixiangYewubanlixiangList(shishiqingdan.getTaskCode());
			shishiqingdan.setYewubanlixiangList(yewubanlixiangList);
		...
		int count = shishiqingdanService.getmeiyoucanshuBumenshixiangShishiqingdanCount();
		if(count>0){
			if(count%10==0){
				pageNum = count/10;
			}else{
	 ...
	 * @return
	 */
	@RequestMapping(value = "bmsxPanduanshifoudengluAndTiaozhuan")
	@ResponseBody
	public String bmsxPanduanshifoudengluAndTiaozhuan(HttpSession session,String taskCode,String rowGuid,String linkAddr,
		...
		String tiaozhuanAddr = "gerenfarenlogin";
		if(member != null){ //如果已经登录了
			//个人已经登录并且这个事项个人可以办理
			//记录我的足迹
			String ticket = new LoginController().getticketfromjis(session);
			acceRecordService.accessRecord(member.getLoginname(), member.getType(), rowGuid, taskname, webid, "部门事项点击在线办理");
			if(serverType.indexOf("1")>-1&&(("10".equals(member.getType())||"1".equals(member.getType())))){
			...
			//个人已经登录并且这个事项个人不可以办理，只能法人进行办理
			if(serverType.indexOf("1")<0&&("10".equals(member.getType())||"1".equals(member.getType()))){
				tiaozhuanAddr="zhinengfarenjinxingbanli";
			}
			//法人已经登录并且这个事项法人可以办理
			if(!"1".equals(serverType)&&("20".equals(member.getType())||"2".equals(member.getType()))){
				if(linkAddr.indexOf("?")>-1){
					tiaozhuanAddr = linkAddr+"&sid="+taskCode+"&spbs="+spbs+"&ticket="+ticket;
				}else{
					tiaozhuanAddr = linkAddr+"&sid="+taskCode+"&spbs="+spbs+"&ticket="+ticket;
			...
			//法人已经登录并且这个事项法人不可以办理，只能个人进行办理
			if("1".equals(serverType)&&("20".equals(member.getType())||"2".equals(member.getType()))){
				tiaozhuanAddr="zhinenggerenjinxingbanli";
			}
		}else{ //如果还没有登录
			//如果只能个人办理
			if("1".equals(serverType)){
				tiaozhuanAddr = "gerenlogin";
			}
			//如果只能个人办理
			if(serverType.indexOf("1")==-1){
				tiaozhuanAddr = "farenlogin";
			}
			if(!"1".equals(serverType)&&serverType.indexOf("1")>-1){
				tiaozhuanAddr = "gerenfarenlogin";
			}
	 ...
	 * @return
	 */
	@RequestMapping(value = "bmsxzxts")
	@ResponseBody
	public String zxtsOnline(HttpSession session){
		Member member  = null;
		if(session.getAttribute("token") != null && !"".equals(session.getAttribute("token"))){
			member = (Member) CacheUtil.getValue(session.getAttribute("token").toString(),
					GszwStaticValues.USERINFO);
