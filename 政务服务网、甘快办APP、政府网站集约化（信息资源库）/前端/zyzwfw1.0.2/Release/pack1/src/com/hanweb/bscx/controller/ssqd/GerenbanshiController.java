 ...
 */
@Controller
@RequestMapping("ssqd/gr")
public class GerenbanshiController {
	@Autowired
	...
	public ModelAndView grlist(String webid,String gerentheme) {
		ModelAndView modelAndView = new ModelAndView();
		if(!FilterUtil.isSqlOK(webid) || NumberUtil.getInt(webid) <= 0 || !FilterUtil.isNumeric(webid)||!FilterUtil.isSqlOK(gerentheme)){
            RedirectView redirectView = new RedirectView(BaseInfo.getContextPath() +"/error/error.do");
            modelAndView.setView(redirectView);
            return modelAndView;
        }
		List<Theme> themelist = (List) CacheUtil.getValue("ziranrentheme",
				GszwStaticValues.ZIRANRENTHEM);
		DataSourceSwitch.change("2");
		List<Dept> deptlist = shishiqingdanService.getUserDeptsNameAndCode(webid);
		
		List<Shishiqingdan> typelist=shishiqingdanService.getUserTypeIdAanName(webid);
		//获取js
		String top="";
		String banner="";
		String nav="";
		...
		modelAndView = new ModelAndView("gszw/bscx/ssqd/gerenbanshi/gr_list");
		...
		modelAndView.addObject("gerentheme", gerentheme);
	 ...
	 * @return
	 */
	@RequestMapping(value = "grdown")
	public ModelAndView grdown(String gerenthemeId,String deptCode,String isonline,String keyword,
		...
		ModelAndView modelAndView = new ModelAndView();
		DataSourceSwitch.change("2");
		List<Shishiqingdan> shishiqingdanlist = shishiqingdanService.getGerenShishiqingdanList(gerenthemeId,typeId,deptCode,
		...
		for(Shishiqingdan shishiqingdan:shishiqingdanlist){
			List<Yewubanlixiang> yewubanlixiangList = shishiqingdanService.getGerenYewubanlixiangList(shishiqingdan.getTaskCode());
			shishiqingdan.setYewubanlixiangList(yewubanlixiangList);
			shishiqingdanHavaYewubanlixianglist.add(shishiqingdan);
		}
		int count = shishiqingdanService.getGerenShishiqingdanCount(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		if(count>0){
		...
		int count1 = shishiqingdanService.getGerenShishiqingdanCount1(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		int count2 = shishiqingdanService.getGerenShishiqingdanCount2(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		int count3 = count1-count2;
		int gerenShishiqingdanAndYewubanlixiangcount = shishiqingdanService.getGerenShishiqingdanCount(gerenthemeId,deptCode,isonline,keyword,webid,typeId);
		modelAndView = new ModelAndView("gszw/bscx/ssqd/gerenbanshi/gr_down");
		...
		modelAndView.addObject("webid", webid);
		modelAndView.addObject("gerenShishiqingdanAndYewubanlixiangcount", gerenShishiqingdanAndYewubanlixiangcount);
		return modelAndView;
	}
	/**
	 * 个人判断是否登录并拼接要跳转的地址
	 * @return
	 */
	@RequestMapping(value = "panduanshifoudengluAndTiaozhuan")
	@ResponseBody
	public String panduanshifoudengluAndTiaozhuan(HttpSession session, String taskCode,String rowGuid, String linkAddr, 
	  ...
	  {
	    if (!(FilterUtil.isSqlOK(taskCode))) {
	      Date date = new Date();
	      System.out.println(date.toString() + ": 输入的参数：" + "taskCode: " + taskCode);
	      return "您输入的参数有问题";
	    }
	    try {
	    ...
	    String tiaozhuanAddr = "";
	    if (member != null) {
	      String ticket = new LoginController().getticketfromjis(session);
	    //记录我的足迹
	      accessRecordService.accessRecord(member.getLoginname(), member.getType(), rowGuid, taskname, webid, "个人办事点击在线办理");
	      if("1".equals(serverType)&&("10".equals(member.getType())||"1".equals(member.getType()))){
				...
				tiaozhuanAddr = "gaishixiangfarenbunengbanli";
			}
	    }
	    else {
	      ...
	      session.setAttribute("serverType", "1");
