      ...
      >
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <div class="list">
              <div class="list-left">机构名称</div>
              <div class="list-right">{{ item.name || "暂无数据" }}</div>
            </div>
            <div class="list">
              <div class="list-left">负责人姓名</div>
              <div class="list-right">{{ item.fzr || "暂无数据" }}</div>
            </div>
            <div class="list">
              <div class="list-left">行政区划代码</div>
              <div class="list-right">{{ item.zxqhdb || "暂无数据" }}</div>
            </div>
            <div class="list">
              <div class="list-left">联系电话</div>
              <div class="list-right">
                <a style="color: #1677ff" :href="'tel:' + item.lxdh">{{
                  item.lxdh || "暂无数据"
                }}</a>
              </div>
            </div>
            <div class="list">
              <div class="list-left">机构地址</div>
              <div class="list-right">{{ item.jgdz || "暂无数据" }}</div>
            </div>
            <div class="list">
              <div class="list-left">机构简介</div>
              <div class="list-right">{{ item.jgjj || "暂无数据" }}</div>
            </div>
          </div>
        </div>
      </van-list>
