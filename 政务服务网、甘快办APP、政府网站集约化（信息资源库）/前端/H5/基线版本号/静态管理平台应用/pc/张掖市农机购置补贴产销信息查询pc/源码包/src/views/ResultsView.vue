    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">姓名或组织名称</td>
            <td colspan="3">{{ item.name || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">乡镇</td>
            <td>{{ item.xz || "暂无数据" }}</td>
             <td class="tab_bt">出厂编号</td>
            <td>{{ item.ccbh || "暂无数据" }}</td>
          </tr>
         <!--  <tr>
            <td class="tab_bt">机具品目</td>
            <td>{{ item.jjpm || "暂无数据" }}</td> 
            <td class="tab_bt">机具型号</td>
            <td>{{ item.jjxh || "暂无数据" }}</td> 
            <td class="tab_bt">出厂编号</td>
            <td>{{ item.ccbh || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">数量</td>
            <td>{{ item.sl || "暂无数据" }}</td>
            <td class="tab_bt">单台中央补贴</td>
            <td>{{ item.dtzybt || "暂无数据" }}</td>
          </tr>-->
          <tr>
            <td class="tab_bt">总中央补贴额</td>
            <td>{{ item.zzybtje || "暂无数据" }}</td>
            <td class="tab_bt">总补贴额</td>
            <td>{{ item.zbte || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
