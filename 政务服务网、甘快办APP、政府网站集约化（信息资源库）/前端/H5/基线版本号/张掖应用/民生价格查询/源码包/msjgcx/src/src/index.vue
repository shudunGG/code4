      ...
      <div class="list_box show_box">
        <ul>
          <li v-for="(item, index) in List" :key="index">
            <div class="info" @click="search2(item)">
              <span class="infoLeft">{{ item.GoodsContent }}</span>
              <span class="address">
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      formdata.append(
        "biz_content",
        JSON.stringify({ param: this.input_text, type: "15" })
      );
    ...
    search2(inpo) {
      ...
      formdata.append(
        "biz_content",
        JSON.stringify({ param: inpo.GoodsContent, type: "16" })
      );
      HTTP.jpass(formdata).then((res) => {
