      ...
      <div class="list_box show_box">
        <ul>
          <li v-for="(item, index) in List" :key="index">
            <p>{{ item.name }}</p>
            <div class="info">
              <span class="infoLeft">区划</span>
              <span class="address"> {{ item.districts }}</span>
            </div>
            <div class="info">
              <span class="infoLeft">企业类型</span>
              <span class="address">
                {{ item.type == 1 ? "政府办公立医院" : "社会事业办" }}</span
              >
            </div>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      formdata.append(
        "biz_content",
        JSON.stringify({ param: this.input_text, type: "56" })
      );
