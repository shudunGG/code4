<template>
  <div class="main">
      ...
      <div class="list_box show_box">
        <ul>
          <li v-for="(item, index) in List" :key="index">
            <div class="info" @click="search2(item.jieZhongDianName)">
              <span class="infoLeft">{{ item.jieZhongDianName }}</span>
              <span class="address">
    ...
    <div :class="this.List.length > 0? 'down1':'down'">
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      formdata.append(
        "biz_content",
        JSON.stringify({ param: this.input_text, type: "25" })
      );
    ...
    search2(inpo) {
      ...
      formdata.append(
        "biz_content",
        JSON.stringify({ param: inpo, type: "26" })
      );
      HTTP.jpass(formdata).then((res) => {
