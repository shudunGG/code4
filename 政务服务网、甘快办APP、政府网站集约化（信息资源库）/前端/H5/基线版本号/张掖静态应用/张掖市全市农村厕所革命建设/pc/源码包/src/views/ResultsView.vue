      ...
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            ...
            <td class="tab_bt">累计建设数量</td>
            <td>{{ item.leiji || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">本年已建设数量</td>
            <td>{{ item.yijianshe || "暂无数据" }}</td>
            <td class="tab_bt">本年任务完成率</td>
            <td>{{ item.wanchenglv ? item.wanchenglv * 100 + '％' : "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
