    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">统计日期</td>
            <td>{{ item.riqi || "暂无数据" }}</td>
            <td class="tab_bt">相关部门</td>
            <td>{{ item.bumen || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">全市残疾人人数</td>
            <td>{{ item.renshu || "暂无数据" }}</td>
            <td class="tab_bt">全市残疾人补贴总额</td>
            <td>{{ item.zonge || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">上年度全市残疾人补贴总额</td>
            <td colspan="3">{{ item.shangjiduzonge || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
        ...
        this.params = {
          ...
          map: sm2.encrypt(
            JSON.stringify({
              riqi: JSON.stringify({
                type: 2,
                content: this.formData.riqi.replace(/\s+/g, ""),
                left: "",
                right: "",
              }),
            })
