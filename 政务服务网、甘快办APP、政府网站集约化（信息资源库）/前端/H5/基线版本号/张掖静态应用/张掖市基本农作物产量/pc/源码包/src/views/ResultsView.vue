      ...
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            ...
            <td>{{ item.nianfen || "暂无数据" }}</td>
            <td class="tab_bt">瓜类产量</td>
            <td>{{ item.gualei ? num(item.gualei) : "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">粮食产量</td>
            <td>{{ item.liangshi ? num(item.liangshi) : "暂无数据" }}</td>
            <td class="tab_bt">油料产量</td>
            <td>{{ item.youliao ? num(item.youliao) : "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">蔬菜产量</td>
            <td colspan="3">{{ item.shucai ? num(item.shucai) : "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
          ...
          map: sm2.encrypt(
            JSON.stringify({
              nianfen: JSON.stringify({
                type: 2,
                content: this.formData.nianfen.replace(/\s+/g, ""),
                left: "",
                right: "",
              }),
            })
