      ...
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            ...
            <td class="tab_bt">填报部门</td>
            <td>{{ item.bumen || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">全市养老机构总数</td>
            <td>{{ item.zongshu || "暂无数据" }}</td>
            <td class="tab_bt">设置床位总数</td>
            <td>{{ item.chuangwei || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">护理型床位数量</td>
            <td>{{ item.hulixing || "暂无数据" }}</td>
            <td class="tab_bt">护理型床位数量占比</td>
            <td>{{ item.zhanbi || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
          ...
          map: sm2.encrypt(
            JSON.stringify({
              bumen: JSON.stringify({
                type: 2,
                content: this.formData.bumen.replace(/\s+/g, ""),
                left: "",
                right: "",
              }),
            })
