    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          ...
          <tr>
            <td class="tab_bt">低保对象总人数</td>
            <td>{{ item.zongrenshu || "暂无数据" }}</td>
            <td class="tab_bt">城市低保对象人数</td>
            <td>{{ item.chengshi || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">农村低保对象人数</td>
            <td colspan="3">{{ item.nongcun || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
