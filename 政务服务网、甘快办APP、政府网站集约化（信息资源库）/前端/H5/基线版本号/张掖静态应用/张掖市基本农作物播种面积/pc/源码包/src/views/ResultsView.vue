    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">统计年份</td>
            <td>{{ item.nainfen || "暂无数据" }}</td>
            <td class="tab_bt">年末耕地面积</td>
            <td>{{ item.gengdi ? num(item.gengdi) : "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">蔬菜面积</td>
            <td>{{ item.shucai ? num(item.shucai) : "暂无数据" }}</td>
            <td class="tab_bt">瓜类面积</td>
            <td>{{ item.gualei ? num(item.gualei) : "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">粮食面积</td>
            <td>{{ item.liangshi ? num(item.liangshi) : "暂无数据" }}</td>
            <td class="tab_bt">油料面积</td>
            <td>{{ item.youliao ? num(item.youliao) : "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
          ...
          map: sm2.encrypt(
            JSON.stringify({
              nainfen: JSON.stringify({
                type: 2,
                content: this.formData.nainfen.replace(/\s+/g, ""),
                left: "",
                right: "",
              }),
            })
