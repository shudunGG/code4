    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">区县</td>
            <td>{{ item.quxian || "暂无数据" }}</td>
            <td class="tab_bt">养殖场户名称</td>
            <td>{{ item.name || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">设计存栏规模</td>
            <td>{{ item.cunlan || "暂无数据" }}</td>
            <td class="tab_bt">设计年出栏规模</td>
            <td>{{ item.chulan || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">养殖场地址</td>
            <td>{{ item.dizhi || "暂无数据" }}</td>
            <td class="tab_bt">养殖畜种</td>
            <td>{{ item.chuzhong || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">养殖种类</td>
            <td>{{ item.zhonglei || "暂无数据" }}</td>
            <td class="tab_bt">主要品种</td>
            <td>{{ item.pinzhong || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">设计年饲养规模</td>
            <td colspan="3">{{ item.niansiyang || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <el-pagination
          ...
          map: sm2.encrypt(
            JSON.stringify({
              quxian: JSON.stringify({
                type: 2,
                content: this.formData.quxian.replace(/\s+/g, ""),
                left: "",
                right: "",
              }),
            })
