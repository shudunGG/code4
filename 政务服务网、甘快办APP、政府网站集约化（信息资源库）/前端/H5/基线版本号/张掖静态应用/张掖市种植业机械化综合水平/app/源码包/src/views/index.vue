<template>
  <div class="container-wrap">
    ...
    <div class="card-wrap" style="margin-top: 110px">
      ...
      <div class="card-list">
        <div class="card-list-title">统计年份</div>
    ...
    <van-popup v-model="show" position="bottom">
      <van-picker show-toolbar :columns="columns" @cancel="cancel" @confirm="confirm" />
    </van-popup>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.columns = yearsArray.map(item => {
      return item + '年'
    })
  },
  mounted() {
  ...
  methods: {
    ...
    queryList() {
      ...
      if (/[\s]/g.test(this.nianfen)) {
        this.nianfen = this.nianfen.replace(/\s+/g, '');
      }
      Toast({
        message: '加载中...',
        duration: 0,
        ...
        res => {
          ...
          if (res.dataList && res.dataList.length > 0) {
            ...
            this.$router.push({
              path: '/listDetail',
              query: {
                name: JSON.stringify(this.nianfen),
              },
            });
          } else {
            Toast({
