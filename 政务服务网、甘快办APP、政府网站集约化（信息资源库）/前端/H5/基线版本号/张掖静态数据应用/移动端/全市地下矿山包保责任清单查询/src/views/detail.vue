      ...
      >
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <table>
              <tr>
                <td colspan="4">张掖市2023年全市地下矿山包保责任清单</td>
              </tr>
              <tr>
                <td class="tab_bt">矿山名称</td>
                <td style="white-space:pre-line">{{ item.mc || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">所属市县（区）</td>
                <td style="white-space:pre-line">{{item.sx || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">生产状态</td>
                <td>{{ item.zt || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">开采矿种</td>
                <td style="white-space:pre-line">{{ item.kz || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">设计生产规模（万吨）</td>
                <td>{{ item.gm || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">安全等级</td>
                <td>{{ item.dj || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">日常监管主体</td>
                <td>{{ item.jg || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">备  注</td>
                <td>{{ item.bz || '暂无数据' }}</td>
              </tr>
            </table>
          </div>
  ...
  methods: {
    renderList() {
      ...
      this.list.forEach((item) => {
        item.mc = item.mc.replace(/<br\/>/g,"\n")
        item.sx = item.sx.replace(/<br\/>/g,"\n")
        item.kz = item.kz.replace(/<br\/>/g,"\n")
      })
    ...
    queryList(value) {
      ...
      IndexApi.queryList(params,'b7f4b972fd2b4847b449d2ff491d6fb9')
        .then(res => {
