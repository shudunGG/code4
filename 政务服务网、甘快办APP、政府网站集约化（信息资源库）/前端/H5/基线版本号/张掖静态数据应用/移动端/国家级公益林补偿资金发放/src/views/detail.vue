      ...
      >
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <table>
              <tr>
                <td colspan="4">张掖市国家级公益林补偿资金发放</td>
              </tr>
              <tr>
              ...
              <tr>
                <td class="tab_bt">管护模式</td>
                <td>{{ item.ms || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">户主姓名</td>
                <td>{{ item.hz || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">林木使用权属</td>
                <td>{{ item.qs || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">补偿面积（亩）</td>
                <td>{{ item.bcmj || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">补偿标准（元/亩）</td>
                <td>{{ item.bcbz || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">补偿劳务费金额（元）</td>
                <td>{{ item.je || '暂无数据' }}</td>
              </tr>
              <tr>
...
export default {
  ...
  methods: {
    ...
    queryList(value) {
      ...
      IndexApi.queryList(params,'6d32e45c1db74ec0a805661ded746a58')
        .then(res => {
