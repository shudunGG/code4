      ...
      <van-list v-model="loading" offset="10" :error.sync="error" :immediate-check="false" @load="onLoad"
        ...
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <table>
              <tr>
                <td colspan="4">张掖市市发证取水许可信息</td>
              </tr>
              <tr>
                <td class="tab_bt">取水权人名称</td>
                <td>{{ item.mc || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">取水权人代码</td>
                <td>{{ item.dm || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">许可证编码</td>
                <td>{{ item.bm || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">许可水量（万立方米）</td>
                <td>{{ item.sl || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">取水用途</td>
                <td>{{ yt(item.yt) || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">有效期起</td>
                <td>{{ item.ks || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">有效期至</td>
                <td>{{ item.js || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">有效年限</td>
                <td>{{ item.nx || '暂无数据' }}</td>
              </tr>
            </table>
          </div>
...
export default {
  ...
  methods: {
    ...
    queryList(value) {
      ...
      IndexApi.queryList(params, '3cfad260831347b79f3550e2b05daf81 ')
        .then(res => {
