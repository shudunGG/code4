        ...
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <table>
              ...
              <tr>
                <td class="tab_bt">命名主体</td>
                <td>{{ item.zt || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">实施单位</td>
                <td style="white-space:pre-line">{{ item.dw || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">获批时间</td>
                <td>{{ item.sj || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">备注</td>
                <td style="white-space:pre-line">{{ item.bz || '暂无数据' }}</td>
              </tr>
            </table>
          </div>
        </div>
  ...
  methods: {
    renderList() {
      ...
      this.list.forEach((item) => {
        item.bz = item.bz.replace(/<br\/>/g, "\n")
        item.dw = item.dw.replace(/<br\/>/g, "\n")
      })
    ...
    queryList(value) {
      ...
      IndexApi.queryList(params, '8a654d4dfd084f3a85714a5620cce636')
        .then(res => {
