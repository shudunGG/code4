      ...
      <van-list v-model="loading" offset="10" :error.sync="error" :immediate-check="false" @load="onLoad"
        ...
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <table>
              <tr>
                <td colspan="4">张掖市农业农村局中初级职称专业技术人员</td>
              </tr>
              <tr>
              ...
              <tr>
                <td class="tab_bt">民族</td>
                <td >{{ item.mz || '暂无数据' }}</td>
              </tr>
              <tr>
                <td class="tab_bt">职称资格</td>
                <td>{{ item.zg || '暂无数据' }}</td>
              </tr>
              <tr>
...
export default {
  ...
  methods: {
    ...
    queryList(value) {
      ...
      IndexApi.queryList(params, 'f270638f254246818ae0d48053eaa920')
        .then(res => {
