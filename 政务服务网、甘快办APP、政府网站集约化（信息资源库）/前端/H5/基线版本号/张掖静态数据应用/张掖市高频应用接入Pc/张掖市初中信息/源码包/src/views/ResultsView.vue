    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">学校名称</td>
            <td>{{ item.name || "暂无数据" }}</td>
            <td class="tab_bt">学段</td>
            <td>{{ item.xueduan || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">学校位置</td>
            <td colspan="3">{{ item.weizhi || "暂无数据" }}</td>
            <!-- <td class="tab_bt">运输企业名称</td>
            <td>{{ item.qiyemingcheng || "暂无数据" }}</td> -->
          </tr>
          <tr>
            <td class="tab_bt">学校简介</td>
            <td colspan="3">{{ item.jianjie || "暂无数据" }}</td>
            <!-- <td class="tab_bt">运输企业名称</td>
            <td>{{ item.qiyemingcheng || "暂无数据" }}</td> -->
          </tr>
          <tr>
            <td class="tab_bt">负责人</td>
            <td >{{ item.fuzeren || "暂无数据" }}</td>
            <td class="tab_bt">联系方式</td>
            <td>{{ item.lianxifangshi || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">招生区域</td>
            <td colspan="3">{{ item.zhaoshengquyu || "暂无数据" }}</td>
            <!-- <td class="tab_bt">运输企业名称</td>
            <td>{{ item.qiyemingcheng || "暂无数据" }}</td> -->
          </tr>
        </table>
