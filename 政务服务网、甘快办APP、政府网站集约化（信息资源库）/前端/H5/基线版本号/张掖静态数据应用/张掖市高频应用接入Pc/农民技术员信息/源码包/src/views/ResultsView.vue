      ...
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            ...
            <td>{{ item.xingbie || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">民族</td>
            <td>{{ item.minzu || "暂无数据" }}</td>
            <td class="tab_bt">有效范围</td>
            <td>{{ item.youxiaofanwei || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">职称等级</td>
            <td colspan="3">{{ item.zhichengdengji || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <!--分页区域-->
