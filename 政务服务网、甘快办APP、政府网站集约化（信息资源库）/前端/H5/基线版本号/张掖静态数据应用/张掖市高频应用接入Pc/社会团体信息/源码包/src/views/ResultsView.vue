    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">社会组织名称</td>
            <td>{{ item.name || "暂无数据" }}</td>
            <td class="tab_bt">法定代表人姓名</td>
            <td>{{ item.faren || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">联系人</td>
            <td>{{ item.lianxiren || "暂无数据" }}</td>
            <td class="tab_bt">联系电话</td>
            <td>{{ item.lianxidianhua || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">住所的详细地址</td>
            <td colspan="3">{{ item.dizhi || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">业务范围</td>
            <td colspan="3">{{ item.yewufanwei || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <!--分页区域-->
