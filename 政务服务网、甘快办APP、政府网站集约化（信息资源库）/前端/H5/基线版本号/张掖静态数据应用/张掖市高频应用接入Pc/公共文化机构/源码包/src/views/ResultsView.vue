    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">单位名称</td>
            <td>{{ item.mingcheng || "暂无数据" }}</td>
            <td class="tab_bt">单位性质</td>
            <td>{{ item.xingzhi || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">单位地址</td>
            <td >{{ item.dizhi || "暂无数据" }}</td>
            <td class="tab_bt">咨询电话</td>
            <td>{{ item.dianhua || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">是否免费开放</td>
            <td >{{ item.mianfei || "暂无数据" }}</td>
            <td class="tab_bt">开放时间</td>
            <td>{{ item.shijian || "暂无数据" }}</td>
          </tr>
  ...
  methods: {
    ...
    async selectResult(params) {
      if (this.formData.name) {
        this.params.mingcheng = encodeURIComponent(
          JSON.stringify({
            type: 2,
