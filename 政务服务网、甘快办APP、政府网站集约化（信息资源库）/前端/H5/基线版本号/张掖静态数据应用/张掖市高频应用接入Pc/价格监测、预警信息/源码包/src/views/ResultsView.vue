    ...
    <div class="result">
      <template>
        <table v-for="(item, index) in resultsObj.dataList" :key="index">
          <tr>
            <td class="tab_bt">商品名称</td>
            <td>{{ item.name || "暂无数据" }}</td>
            <td class="tab_bt">月份</td>
            <td>{{ item.yuefen || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">规格型号</td>
            <td>{{ item.xinghao || "暂无数据" }}</td>
            <td class="tab_bt">单位</td>
            <td>{{ item.danwei || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">本月价格</td>
            <td>{{ item.benyuejiage || "暂无数据" }}</td>
            <td class="tab_bt">上一月价格</td>
            <td>{{ item.shangyiyue || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">上两月价格</td>
            <td>{{ item.shangliangyue || "暂无数据" }}</td>
            <td class="tab_bt">环比</td>
            <td>{{ item.huanbi || "暂无数据" }}</td>
          </tr>
          <tr>
            <td class="tab_bt">同比</td>
            <td>{{ item.tongbi || "暂无数据" }}</td>
            <td class="tab_bt">备注</td>
            <td>{{ item.beizhu || "暂无数据" }}</td>
          </tr>
        </table>
        <div class="block">
          <!--分页区域-->
