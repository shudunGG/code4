<template>
  <div class="page">
    <div class="page_wrap">
      <!-- 循环展示医院报告列表 -->
      <div class="container-wrap" v-for="(item, index) in cfcxmzzxxList" :key="index" @click="queryList(item)">
        <div class="pickertitle">
          <div class="title">{{ item.hospitalName || '/' }}</div>
          <div class="time">{{ item.date || '/'}}</div>
        </div>
        <div class="pickertext">
          <div>报告类型：{{ item.reportType==0? "检验报告" : "检验报告" }}</div>
...
<script>
import tools, { name, IdNumberDesensitization, formatDate, replaceAll } from '../tools/tool'
...
export default {
  ...
  methods: {
    ...
    getcfcxmzzxx () {
      ...
      let params = {
        results: tools.encryptSM2(JSON.stringify({
          authCode: '',
          token:
            this.$store.state.userInfo.tokenNo || "naturalToken7156850363506678594320084065620021177651618483264323524208036061", // 使用的token
      ...
      serveAPI
        ...
        .then(res => {
          console.log(res, '临泽县医院报告列表查询')
          if (res.code == 200 && res.success && JSON.parse(tools.decryptSM2(res.data)).length > 0) {
            Toast.clear()
    ...
    queryList (val) {
      console.log(val)
      this.$router.push({ name: 'infoDetail', query: { info: JSON.stringify(val) } })
    }
  }
}
</script>
