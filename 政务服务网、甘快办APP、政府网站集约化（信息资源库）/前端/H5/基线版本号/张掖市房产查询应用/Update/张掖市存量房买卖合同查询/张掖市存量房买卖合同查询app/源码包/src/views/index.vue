<template>
  <div class="app">
    ...
    <div class="card-warp">
      ...
      /> -->
      <!-- <div class="text-top" style="margin-top: 12px">
        <div class="left">卖房人姓名</div>
      </div>
      <van-field
        v-model="sellerxm"
        placeholder="请输入卖房人姓名"
        clearable
        maxlength="50"
      />
      <div class="text-top" style="margin-top: 12px">
        <div class="left">卖房人身份证号</div>
      </div>
      <van-field
        v-model="sellerzjhm"
        placeholder="请输入卖房人身份证号"
        clearable
        maxlength="50"
      /> -->
      <div class="text-top" style="margin-top: 12px">
        <div class="left">
          <span style="color: #ee0a24; margin-right: 2px">*</span>购房人姓名
        </div>
      </div>
      <van-field
        v-model="buyerxm"
      ...
      <div class="text-top" style="margin-top: 12px">
        <div class="left">
          <span style="color: #ee0a24; margin-right: 2px">*</span>购房人身份证号
        </div>
      </div>
      <van-field
        v-model="buyerzjhm"
...
import tools, { name, IdNumberDesensitization } from "../tools/tool.js";
export default {
  name: "indexName",
  setup() {
    const obj = reactive({
    ...
    const getUser = async () => {
      ...
      const userInfo = {
        ...
        buyerxm: res.data.userName,
        buyerzjhm: res.data.userCertNum,
      };
      obj.buyerxm = name(res.data.userName);
      obj.buyerzjhm = IdNumberDesensitization(res.data.userCertNum);
    ...
    const getList = async () => {
      ...
      // }
      // if (obj.sellerxm != "" && obj.sellerzjhm == "") {
      //   showToast("请输入卖方证件号码");
      //   return;
      // }
      // if (obj.sellerzjhm != "" && obj.sellerxm == "") {
      //   showToast("请输入卖方姓名");
      //   return;
      // }
      // if (obj.sellerxm == "" && obj.sellerzjhm == "") {
      //   delete obj.sellerxm;
      //   delete obj.sellerzjhm;
      // }
      let specialKey =
        "/[`~!@#$^&*=|{}':;',\\\\\\[\\]\\.<>\\/?~！@#￥……&%*（）|{}【】'；：\"\"'。，、？\\]/";
      for (let i = 0; i < obj.clfhtbh.length; i++) {
        if (specialKey.indexOf(obj.clfhtbh.substr(i, 1)) != -1) {
          showToast("输入不能含有特殊字符与空格");
          return;
        }
      }
      const regex = /\s/;
      if (regex.test(obj.clfhtbh)) {
        showToast("输入不能含有特殊字符与空格");
        return;
      }
      const params = {
        params: tools.sm2Encrypt1(
          JSON.stringify({
            clfhtbh: obj.clfhtbh,
            // clfhtbh: "202009120152",
            // fwzl: "民乐县生态宜居小镇36号住宅楼（36）幢八层3-802",
            // fwzl: obj.fwzl,
            buyerxm: JSON.parse(store.userInfo).buyerxm,
            buyerzjhm: JSON.parse(store.userInfo).buyerzjhm,
            // sellerxm: obj.sellerxm,
            // sellerzjhm: obj.sellerzjhm,
