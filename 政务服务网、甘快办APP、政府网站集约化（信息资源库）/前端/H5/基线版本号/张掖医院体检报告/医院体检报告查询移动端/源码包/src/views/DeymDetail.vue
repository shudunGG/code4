    ...
    <div class="top">
      ...
      <div class="container-bottom">
        <div class="card-wrap" v-for="(item, index) in dataDetail" :key="index">
          <div class="list">
            <div class="list-left">医院代码</div>
            <div class="list-right">{{ item.hospitalcode || "暂无数据" }}</div>
          </div>
          <div class="list">
            <div class="list-left">电话号码</div>
            <div class="list-right">{{ phone(item.mobile) || "暂无数据" }}</div>
          </div>
          <div class="list">
            <div class="list-left">姓名</div>
            <div class="list-right">{{ item.patientname || "暂无数据" }}</div>
          </div>
          <div class="list">
            <div class="list-left">性别</div>
            <div class="list-right">{{ item.sex === '1' ? '男' : '女' || "暂无数据" }}</div>
          </div>
          <div class="list">
          ...
          <div class="list">
            <div class="list-left" style="margin-left: 12px">
              <van-button type="default" @click="brow(item.httppdf)">报告预览</van-button>
            </div>
<!--            <div class="list-right" style="margin-right: 12px" v-if="isWeiXin">-->
<!--              <van-button type="primary" @click="downloadPDF(item.httppdf)">报告下载</van-button>-->
<!--            </div>-->
          </div>
        </div>
      </div>
...
import { phone } from '../utils/desensitization'
...
// const dataDetail = ref(store.listDetail.tjrylb)
const dataDetail = Array.isArray(store.listDetail.tjrylb) ? ref(store.listDetail.tjrylb) : [store.listDetail.tjrylb]
...
onMounted(() => {
  ...
  // if (userAgent.indexOf('micromessenger') !== -1 || userAgent.indexOf('alipayclient') !== -1) { // 微信小程序支付宝小程序
  //   isWeiXin.value = true
  // }
})
