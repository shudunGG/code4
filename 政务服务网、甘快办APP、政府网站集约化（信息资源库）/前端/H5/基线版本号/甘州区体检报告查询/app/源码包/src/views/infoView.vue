<template>
  <div class="page">
    <div class="page_wrap">
      ...
      >
        <div class="pickertitle">
          ...
          <div class="time">{{ item.MAINCHECKTIME || "/" }}</div>
        </div>
    ...
    <!-- 底部 -->
    <div class="footer">
      <div class="footer-content">本服务由甘州区提供</div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getcfcxmzzxx() {
      ...
      // };
      let params = {
        tokenNo: tools.encryptSM2(this.$store.state.userInfo.tokenNo),
        authCode: "",
        size: "10",
        current: "1",
        orderBy: "",
        ...
        .then((res) => {
          ...
          try {
            if (
              res.code == 200 &&
              res.success &&
              JSON.parse(tools.decryptSM2(res.data)).data.list.length > 0
            ) {
              Toast.clear();
              let result = JSON.parse(tools.decryptSM2(res.data)).data.list;
