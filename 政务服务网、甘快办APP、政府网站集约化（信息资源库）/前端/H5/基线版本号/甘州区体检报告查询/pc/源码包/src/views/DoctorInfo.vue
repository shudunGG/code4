    ...
    <div class="content_from">
      ...
      >
        <div class="card_border">
          <img :src="item.hospitalPictureBig" alt="" srcset="" />
          <div class="card_text">
            ...
            <div>检查项目：{{ item.CONFIGNAME }}</div>
          </div>
        </div>
        <div class="card_time">{{ item.MAINCHECKTIME }}</div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getcfcxmzzxx() {
      ...
      //       dataType: `mlyy`,
      //       patientName: "周雪琴",
      //       cardNo: "622223198803124621",
      //       phone: "18193678155",
      //     })
      //   ),
      // };
      let params = {
        tokenNo: tools.encrypt(this.$store.state.userInfo.token),
        authCode: tools.encrypt(this.$store.state.userInfo.authCode),
        size: '10',
        current: '1',
        orderBy: ''
      }
      serveAPI
        .queryList("zyyyjkrzmpgab", "zygzPhysical", params)
        .then((res) => {
          console.log(res, "甘州区医院报告列表查询");
          console.log(JSON.parse(tools.decrypt(res.data)))
          
          if (
            res.code == 200 &&
            res.success &&
            JSON.parse(tools.decrypt(res.data)).data?.list?.length > 0
          ) {
            this.loading = false;
            let result = JSON.parse(tools.decrypt(res.data)).data.list;
            console.log(result, "甘州区医院报告列表查询11");
            result.forEach((item) => {
