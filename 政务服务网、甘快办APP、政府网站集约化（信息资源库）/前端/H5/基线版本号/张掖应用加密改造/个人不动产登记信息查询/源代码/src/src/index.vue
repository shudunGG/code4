        ...
        <ul>
          <li class="li_first">
            ...
            <form @submit.prevent="formSubmit" action="javascript:return true" style="width: 70%">
              <input type="text" placeholder="请输入持证人姓名" v-model="user" ref="inp" @keydown.enter="search" />
            </form>
          </li>
        </ul>
      </div>
      ...
      <div class="list_box show_box">
        <ul>
          <li v-for="(item, index) in List" :key="index">
            ...
            <div class="info">
              <span class="infoLeft">持证人名称</span>
              <span class="address"> {{ item.xm.replace(/^(.).+$/, '$1*') }}</span>
            </div>
            <div class="info">
...
export default {
  ...
  filters: {
    encodeSFZ(e) {
      return tools.idcardDesensitization(e).idCard
    }
  },
  created() {
    this.getInfo()
  ...
  methods: {
    ...
    async search(pageNum) {
      ...
      let key = this.loginName + '_6207grbdcdjxx'
      let param = {
        key
      }
      this.searchParams = {
      ...
      Jmas(this.searchParams).then(res => {
        if (JSON.parse(res.data).success) {
          ...
          console.log('num++++++', this.searchNum)
          if (this.searchNum > 5) {
            ...
            this.params = {
              ...
              biz_content: JSON.stringify({
                columnId: tools.encrypt('724e9ac298e9476f94565d17c8927b7e', publicKey, 1),
    ...
    async searchMore(pageNum) {
      ...
      if (this.user) {
        ...
        this.params = {
          app_id: 'cmsnzjk',
          interface_id: 'cmslistInfonzjk',
          header: '',
          biz_content: JSON.stringify({
            columnId: '724e9ac298e9476f94565d17c8927b7e',
      ...
      let res = await Jmas(this.params)
      this.$toast.clear()
      console.log(res, '这是加载更多的res')
      let resOne = JSON.parse(res.data)
      console.log(resOne.data.dataResult.dataList, '#####')
      if (resOne.data.dataResult) {
        if (resOne.data.dataResult.totalCount > 0) {
          if (this.pageNum == 1) {
            this.List = resOne.data.dataResult.dataList
          } else {
            this.List.push(...resOne.data.dataResult.dataList) // 将数据放入list中
          }
          if (this.List.length >= resOne.data.dataResult.totalCount) {
            this.moreThan = false
            // 结束加载状态
          } else {
            this.moreThan = true
