...
import { useRoute } from "vue-router";
 ...
 const getList = async()=>{
    const data = {
        'params':sm2Encrypt(JSON.stringify({"normGuid":"7b9cb06a-3de2-4cf7-9e8b-c3503751b09d",
    ...
    if(res.code == 200){
        ...
        if(result1.custom.result.cockpitData.length){
           if(title == '国家级'){
            list.push(...result1.custom.result.cockpitData.filter(item => item.xmdj == '国家级'))
           }else{
             list.push(...result1.custom.result.cockpitData.filter(item => item.xmdj == '省级'))
           }
        }
    }
}
