        ...
        <div class="container-bottom">
          <div class="card-wrap" v-for="(item, index) in list" :key="index">
            <div class="list">
              ...
              <div class="list-right">{{ item.sqmc || "暂无数据" }}</div>
            </div>
            <div class="list">
              <div class="list-left">机构地址</div>
              <div class="list-right">{{ item.sqdz || "暂无数据" }}</div>
            </div>
            <div class="list">
              <div class="list-left">联系电话</div>
              <div class="list-right">{{ item.lxfs || "暂无数据" }}</div>
            </div>
          </div>
        </div>
      </van-list>
    </div>
    <div class="message">本服务由兰州市提供</div>
  </div>
</template>
    ...
    queryList(value) {
      ...
      if (this.selectArr != "") {
        params = {
          pageNum: this.pageNo,
          pageSize: 20,
          dz: encodeURIComponent(
            JSON.stringify({
              type: 2,
              content: this.selectArr,
              left: "",
              right: "",
            })
          ),
