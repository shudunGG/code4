<template>
  <div class="concent_info_picker">
    <div class="upload" v-for="(item, i) in fileItem" :key="i">
      <div class="picker_t">
        <span>{{ item.Ismust == 2 ? '*' : '' }} </span> {{ item.filename }}
      </div>
      <div class="picker">
        <div class="picker_y" v-for="(items, index) in item.fileList" :key="index">
          <van-image width="125" height="125" :src="items.url" @click="previewImage(items, index)" />
          <div class="cross" @click="deleteFile(item, i, index)"><van-icon name="cross" /></div>
        </div>
...
export default {
  ...
  created() {
    console.log(this.maxCount, this.fileFormat, this.fileCount, this.fileItem, 'maxCount,fileFormat,fileCount,fileItem')
  },
  computed: {
    // max() {
    ...
    uploadFile(item, i) {
      ...
      lightAppJssdk.media.chooseImage({
        ...
        success: function (data) {
          ...
          item.fileList.push(...data.picPath.map(url => ({ url })))
          that.$forceUpdate()
          console.log(that.fileItem, 'item.fileList')
