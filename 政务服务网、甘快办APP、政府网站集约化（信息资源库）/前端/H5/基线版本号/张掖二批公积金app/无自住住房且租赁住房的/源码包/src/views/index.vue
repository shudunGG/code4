      ...
      <div class="concent">
        ...
        <div class="concent_info">
          ...
          <div class="concent_info_picker">
            <div class="picker_t"><span>*</span>申请提取金额（元）：</div>
            <van-field
              maxlength="30"
          ...
          <div class="concent_info_picker">
            <div class="picker_t"><span>*</span>申请人：</div>
            <van-field
              maxlength="30"
              v-model="accountInfo.xingmingtm"
              readonly
              placeholder="请填写申请提取金额"
            />
          </div>
          ...
          <!-- <div class="concent_info_picker">
            <div class="picker_t"><span>*</span>购房人姓名：</div>
            <van-field
              maxlength="30"
              v-model="basicInfo.gfrxm"
              placeholder="请填写购房人姓名"
            />
          </div>
          <div class="concent_info_picker">
            <div class="picker_t"><span>*</span>购房人身份证号：</div>
            <van-field
              maxlength="30"
              v-model="basicInfo.gfrsfzh"
...
export default {
  ...
  methods: {
    ...
    validate() {
      const pattern = /^[\u4e00-\u9fa5]+$/;
      const pattern1 = /^[0-9]*$/
      if (!pattern1.test(this.basicInfo.sqtqjexx)) {
        console.log(pattern.test(this.basicInfo.sqtqjexx));
        Toast({
          message: "请输入正确的申请提取金额",
        });
        return;
      }
      if (!pattern.test(this.basicInfo.sqrskhm)) {
        console.log(pattern.test(this.basicInfo.sqrskhm));
        Toast({
          message: "请输入正确的申请人收款户名",
        });
        return;
      }
      if (!pattern1.test(this.basicInfo.sqrskzh)) {
        Toast({
          message: "请输入正确的申请人收款账号",
        });
        return;
      }
    },
    onConfirm(value) {
      ...
      this.basicInfo.TQSD = changDateFormate(value);
      this.showPicker = false;
    },
    onFinish({ selectedOptions }) {
      this.showPickers = false;
      console.log("selectedOptions",selectedOptions);
      this.basicInfo.sqrkhyhmc = selectedOptions.map((option) => option.yhmc)[selectedOptions.length-1];
      this.basicInfo.sqrkhyhdm = selectedOptions.map((option) => option.yhdm)[selectedOptions.length-1];
    },
    onChange(){},
    getBankList() {
      let param = {
      ...
      serveAPI
        ...
        .then((res) => {
          ...
          if (res.code == "00" && res.data) {
            ...
            this.accountInfo.xingmingtm = name(res.data.xingming);
            this.basicInfo.grzh = res.data.grzh;
            this.basicInfo.zjhm = res.data.zjhm;
            // this.basicInfo.sqrkhyhdm = res.data.grckzhkhyhdm;
            // this.basicInfo.sqrkhyhmc = res.data.grckzhkhyhmc;
